...
<%
	...
	/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	String[] mc = {"202", "205", "201", "203", "202", "206"};
	//System.out.println("mc length = " + mc.length);
	//session.setAttribute("mc", "202");

	String[] mcString = {"Battaramulla", "Jaela", "Kiribathgoda", "Panadura", "Rajagiriya", "Collecting Center"};

	double[] target = new double[6], revenue = new double[6], head_count = new double[6],
	tag_head_count = new double[6];
	double[] target_per = new double[6];

	double[] tag_head_count_per = new double[6];
	...
	String[] color1 = new String[6], color2 = new String[6];
	...
	for (int i = 0; i < mc.length; i++) {
		...
		try {
	//System.out.println("mc1 = " + mc[i]);

	conn2.ConnectToDataBase(Integer.parseInt(mc[i]));
	...
	if (i == 0) {
		if (request.getParameter("date_from") != null) {
			...
			date_from = Query.getDateMonthYear("today", conn2);
			date_to = date_from;
		}
		...
		session.setAttribute("date_to", date_to);
			...
			if (mc[i].equals("202") && i == 4) {
				target[i] = Query.getAllTargetAmountCCUnit(date_from, date_to, conn2, "U00109");
				tag_head_count[i] = Query.getAllTargetCountCCUnit(date_from, date_to, conn2, "U00109");
				revenue[i] = Query.getRevenueAllCCUnit(date_from, date_to, select_type1, conn2, "U00109");
				head_count[i] = Query.getCountAllCCUnit(date_from, date_to, select_type1, conn2, "U00109");
			} else {
				if (mc[i].equals("202")) {
					target[i] = Query.getAllTargetAmountCC(date_from, date_to, conn2) - Query.getAllTargetAmountCCUnit(date_from, date_to, conn2, "U00109");
					tag_head_count[i] = Query.getAllTargetCountCC(date_from, date_to, conn2)
							- Query.getAllTargetCountCCUnit(date_from, date_to, conn2, "U00109");					

					revenue[i] = Query.getRevenueAllCC(date_from, date_to, conn2) - Query.getRevenueAllCCUnit(date_from, date_to, select_type1, conn2, "U00109");
					head_count[i] = Query.getCountAllCC(date_from, date_to, conn2) - Query.getCountAllCCUnit(date_from, date_to, select_type1, conn2, "U00109");
				} else {
					target[i] = Query.getAllTargetAmountCC(date_from, date_to, conn2);
					tag_head_count[i] = Query.getAllTargetCountCC(date_from, date_to, conn2);					

					revenue[i] = Query.getRevenueAllCC(date_from, date_to, conn2);
					head_count[i] = Query.getCountAllCC(date_from, date_to, conn2);
				}
	...
	///////////////////////// Paliyagoda /////////////////////////////////////////////////////////////////////
	target[5] = (target[0] + target[1] + target[2] + target[3]);
	tag_head_count[5] = (tag_head_count[0] + tag_head_count[1] + tag_head_count[2] + tag_head_count[3]);

	//System.out.println("target[5] = " + target[5]);
	//System.out.println("tag_head_count[5] = " + tag_head_count[5]);
	
	//System.out.println("revenue[5] = " + (revenue[5]));
	//System.out.println("tag_head_count[5] = " + tag_head_count[5]);

	if (target[5] > 0) {
		target_per[5] = (revenue[5]) * 100 / target[5];
	}

	if (target[5] < (revenue[5])) {
		color1[5] = "#64E572";
	} else {
		color1[5] = "#F16868";
	}

	if (tag_head_count[5] < (head_count[5])) {
		color2[5] = "#64E572";
	} else {
		color2[5] = "#F16868";
	}

	if (tag_head_count[5] > 0) {
		tag_head_count_per[5] = (head_count[5]) * 100 / tag_head_count[5];
	}
		...
		var myChart1 = Highcharts
				.chart(
						'containerBar1',
						{
							...
							title : {
								text : '<%=select_type1%> Collecting Center Target vs Revenue'
							},
							subtitle : {
							...
							xAxis : {
								...
								categories : [ '<%=mcString[0]%>', '<%=mcString[1]%>',
										'<%=mcString[2]%>',
										'<%=mcString[3]%>', '<%=mcString[4]%>', '<%=mcString[5]%>' ]
							},
							yAxis : {
								//gridLineColor : 'white',
								title : {
							...
							plotOptions : {
								series : {
									...
									dataLabels : {
										...
										x: 0,
						                y: -60,
										format : '<span align="center" style="font-size:20px; color:#f96868; text-align: middle">{point.per:.0f} {point.symbol}</span><br/><span style="font-size:12px">Rs. {point.y:,.0f}</span>'
									}
								},
								column : {
									depth : 100
		...
		var myChart2 = Highcharts
				.chart(
						'containerBar2',
						{
							...
							colors: ['rgb(124, 181, 236)', null],
							title : {
								text : '<%=select_type1%> Collecting Center Target vs Achived Head Count'
							},
							subtitle : {
								text : 'Between <b><u><%=date_from%></u></b> and <b><u><%=date_to%></u></b>'
							},
...
<%}%>
		...
		Highcharts
				.chart(
						'pie_revenue',
						{
							...
							title : {
								text : '<%=select_type1%> Collecting Center Revenue Contribution'
							},
							subtitle : {
	...
	code : '<%=mcString[5]%>',
	name : '<%=mcString[5]%>',
	amount : <%=revenue[5] - revenue[4]%><%if (tot_revenue > 0) {%>,
	y : <%=((revenue[5] - revenue[4]) * 100) / tot_revenue%>
	<%}%>
} ]
							} ]
						});
		...
		Highcharts
				.chart(
						'pie_head_count',
						{
							...
							title : {
								text : '<%=select_type1%> Collecting Center Head Count Contribution'
							},
							subtitle : {
								text : '<b><u><%=date_from%></u></b> and <b><u><%=date_to%></u></b>'
							},
	...
	code : '<%=mcString[5]%>',
	name : '<%=mcString[5]%>',
	amount : <%=(head_count[5] - head_count[4])%><%if (tot_head_count > 0) {%>,
	y : <%=((head_count[5] - head_count[4]) * 100) / tot_head_count%>
	<%}%>
} ]
							} ]
						});
				...
				<div class="content-wrapper">
					...
					<div class="row" style="margin-left: 0px;">
						...
						<button class="btn btn-outline-warning" type="button"
							...
							aria-controls="multiCollapseExample1 multiCollapseExample2 multiCollapseExample3 multiCollapseExample4 multiCollapseExample5">Toggle
							All Centers</button>
						...
						<div>
							<jsp:include page="filter_date.jsp"><jsp:param
									value="dashboardsuperadmincc.jsp" name="action" /><jsp:param
									value="<%=date_from%>" name="date_from" /><jsp:param
									value="<%=date_to%>" name="date_to" /><jsp:param value="no"
									name="sub" /><jsp:param value="yes" name="cc" /></jsp:include></div>
					</div>
					<%
						///////////////////////// Modal - End
					%>
							...
							<div class="col-12 grid-margin">
								<div class="card">
									<div class="card-body">
										...
										<div id="sliders1" style="display: none;">
											<table>
												<tr>
													<td>Alpha Angle</td>
