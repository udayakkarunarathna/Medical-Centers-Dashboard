...
<%
	DecimalFormat df = new DecimalFormat("############.##");
		...
		String service_code = "", service_name = "";
		if (request.getParameter("service_code_name") != null) {
			service_code = request.getParameter("service_code_name").split("~")[0];
			service_name = request.getParameter("service_code_name").split("~")[1];
		}
					...
					<div class="row">
						<%
							String month_arr[] = { "January", "February", "March", "April", "May", "June", "July", "August",
								...
								List<String[]> sna_al = new ArrayList<String[]>();

								sna_al = Query.getSNAYearTargetServicewise(mc, year, service_code);

								session.setAttribute("sna_al", sna_al);

								//System.out.println("sna_al size = " + sna_al.size());
						...
						<div class="col-12 grid-margin">
							<div class="card">
								<div class="card-body">
									<h4 class="card-title">
										Service Target for year
										<%=year%></h4>
									<form method="post" action="SaveSNATarget" id="tar-form">
										<input type="hidden" name="year" value="<%=year%>" />
										<input type="hidden" name="service_code" value="<%=service_code%>" />
										<p class="card-description"><%=service_name%></p>
