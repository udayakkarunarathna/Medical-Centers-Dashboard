			...
			<form style="font-weight: bold;" method="post" action="<%=action%>">
				...
				<div class="modal-body">
					...
					<div class="form-group" align="left">
						<label for="speciality">Speciality</label> <select
							class="form-control form-control" id="speciality"
							style="font-weight: bold; color: green;" name="speciality">
							<option style="font-weight: bold; color: blue;" value="*~*"
								<%if (speciality.equals("*~*")) {%> selected="selected" <%}%>>--
								All --</option>
							<%
								// System.out.println(speciality);
								ArrayList<String[]> speciality_al = Query.getSpeciality(mc);
								session.setAttribute("speciality_al", speciality_al);
								for (int i = 1; i <= speciality_al.size(); i++) {
							%>
							<option
								<%if (speciality.equals(speciality_al.get(i - 1)[0] + "~" + speciality_al.get(i - 1)[1])) {%>
								selected="selected" <%}%>
								value="<%=speciality_al.get(i - 1)[0]%>~<%=speciality_al.get(i - 1)[1]%>"
								style="font-weight: bold; color: green;"><%=speciality_al.get(i - 1)[1]%></option>
							<%
								}
							%>
						</select>
