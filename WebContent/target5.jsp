							...
							<div class="card" style="overflow: scroll;">
								<div class="card-body" style="width: 2000px;">
									...
									<h5 class="card-title" align="left">
										Doctor Speciality Target Patient Count for year
										<%=year%></h5>
									<form method="post" action="SaveSpecTarget" id="tar-form">
										...
										<div class="">
											<table class="table table-hover table-striped">
												<%
												ArrayList<String[]> spec_al = Query.getSpeciality(mc);

														session.setAttribute("spec_al", spec_al);
														...
														al = Query.getSpecialityYearTarget(mc, year);

														String insert_update = "";

														if (al.size() == 0) {
															insert_update = "insert";
															for (int i = 1; i <= (spec_al.size()) * (month_arr.length); i++) {
												...
												<thead>
													<tr>
														<th valign="middle">#</th>
														<th valign="middle">Speciality</th>
														<%
															for (int i = 1; i <= month_arr.length; i++) {
														%>
														<th align="center" style="text-align: center;"><%=month_arr[i - 1]%></th>
												...
												<tbody>
													<%
															for (int i = 1, k = 0; i <= spec_al.size(); i++) {
													%>
													<tr>
														<td><%=i%>.</td>
														<td><%=spec_al.get(i - 1)[1]%><input type="hidden"
															name="spec_<%=i%>" value="<%=spec_al.get(i - 1)[0]%>" /></td>
														<%
															for (int j = 1; j <= month_arr.length; j++, k++) {
														%>
														<td align="center">
															<div class="input-group">
																<input type="text" class="form-control" size="5"
																	name="<%=i%>_<%=j%>" onclick="select()"
																	value="<%=al.get(k)[2]%>"
																	title="<%=month_arr[j - 1]%> Amount - <%=spec_al.get(i - 1)[1]%>"
