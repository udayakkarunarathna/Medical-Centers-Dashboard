		...
		<div class="modal-content">
			...
			<form style="font-weight: bold;" method="post" action="<%=action%>">
				<div class="modal-body">
					<div class="form-group"
						...
						<div class="btn-group btn-group-sm" data-toggle="buttons" style="border: 0px">
							<label
								...
								type="radio" name="select_type1" id="inward" value="INWARD" <%if (select_type1.equals("INWARD")) {%> checked="checked" <%}%>/>Inward&nbsp;&nbsp;</label><label
					...
					<%
						if (!action.equals("dashboard.jsp")) {
					%>
					<div class="form-group" style="margin-bottom: 0px;" align="center">
						<div class="btn-group btn-group-sm" data-toggle="buttons" style="border: 0px">
							<label
								class="btn btn-info <%if (select_type2.equals("Revenue")) {%> active <%}%>"
								for="Rev" style="cursor: pointer;"><input type="radio"
								name="select_type2" id="Rev" value="Revenue" <%if (select_type2.equals("Revenue")) {%> checked="checked" <%}%>/>Revenue&nbsp;&nbsp;</label>

							<label
								class="btn btn-info <%if (select_type2.equals("Transactions")) {%> active <%}%>"
								for="Txn" style="cursor: pointer;"><input type="radio"
								name="select_type2" id="Txn" value="Transactions" <%if (select_type2.equals("Transactions")) {%> checked="checked" <%}%>/>Transactions&nbsp;&nbsp;</label>
							<label
								class="btn btn-info <%if (select_type2.equals("Revenue and Transactions")) {%> active <%}%>"
								for="All" style="cursor: pointer;"><input type="radio"
								name="select_type2" id="All" value="Revenue and Transactions" <%if (select_type2.equals("Revenue and Transactions")) {%> checked="checked" <%}%>/>All&nbsp;&nbsp;</label>
						</div>
					</div>
					<%
						}
