...
<%
	int mc = 0;
		...
		String select_type2_S = "Revenue", curency1 = "Rs. ", curency2 = "(Rs.)", curency3 = "", fraction = "2f";
		...
		// System.out.println("select_type1 com = " + select_type2);
		...
		try {
			...
			if (date_selection.equals("date")) {
				...
				//target = Query.getTargetTransaction("PH", date1, date1, select_type1, select_type2, conn);

				if (select_type2.equals("REV")) {
					income1 = Query.PharmachyRevenueAll(date1, date1, select_type1, "All", conn);
					income2 = Query.PharmachyRevenueAll(date2, date2, select_type1, "All", conn);
					al = Query.PharmacyRevenueBreakdownWC(date1, date1, date2, date2, select_type1, "ASC",
							conn);
				} else if (select_type2.equals("TXN")) {
					income1 = Query.PharmachyCount(date1, date1, select_type1, "All", conn);
					income2 = Query.PharmachyCount(date2, date2, select_type1, "All", conn);
					al = Query.PharmachyCountBreakdownWC(date1, date1, date2, date2, select_type1, "ASC",
							conn);
				}
				...
				/* target = (Query.getTargetTransaction("PH", week1, week11, select_type1, select_type2, conn)
						+ Query.getTargetTransaction("PH", week2, week22, select_type1, select_type2, conn))
						/ 2; */

				if (select_type2.equals("REV")) {
					income1 = Query.PharmachyRevenueAll(week1, week11, select_type1, "All", conn);
					income2 = Query.PharmachyRevenueAll(week2, week22, select_type1, "All", conn);
					al = Query.PharmacyRevenueBreakdownWC(week1, week11, week2, week22, select_type1, "ASC",
							conn);
				} else if (select_type2.equals("TXN")) {
					income1 = Query.PharmachyCount(week1, week11, select_type1, "All", conn);
					income2 = Query.PharmachyCount(week2, week22, select_type1, "All", conn);
					al = Query.PharmachyCountBreakdownWC(week1, week11, week2, week22, select_type1, "ASC",
							conn);
				}
				...
				/* target = Query.getTargetTransaction("PH", (month1 + "-01"), Query.getLastDate(month2, conn),
						select_type1, select_type2, conn) / 2; */

				if (select_type2.equals("REV")) {
					income1 = Query.PharmachyRevenueAll((month1 + "-01"), Query.getLastDate(month1, conn),
							select_type1, "All", conn);
					income2 = Query.PharmachyRevenueAll((month2 + "-01"), Query.getLastDate(month2, conn),
							select_type1, "All", conn);
					al = Query.PharmacyRevenueBreakdownWC((month1 + "-01"), Query.getLastDate(month1, conn),
							(month2 + "-01"), Query.getLastDate(month2, conn), select_type1, "ASC", conn);
				} else if (select_type2.equals("TXN")) {
					income1 = Query.PharmachyCount((month1 + "-01"), Query.getLastDate(month1, conn),
							select_type1, "All", conn);
					income2 = Query.PharmachyCount((month2 + "-01"), Query.getLastDate(month2, conn),
							select_type1, "All", conn);
					al = Query.PharmachyCountBreakdownWC((month1 + "-01"), Query.getLastDate(month1, conn),
							(month2 + "-01"), Query.getLastDate(month2, conn), select_type1, "ASC", conn);
				}
				...
				/* target = Query.getTargetTransaction("PH", (year1 + "-01-01"), (year2 + "-12-31"), select_type1,
						select_type2, conn) / 2; */

				if (select_type2.equals("REV")) {
					income1 = Query.PharmachyRevenueAll((year1 + "-01-01"), (year1 + "-12-31"), select_type1,
							"All", conn);
					income2 = Query.PharmachyRevenueAll((year2 + "-01-01"), (year2 + "-12-31"), select_type1,
							"All", conn);
					al = Query.PharmacyRevenueBreakdownWC((year1 + "-01-01"), (year1 + "-12-31"),
							(year2 + "-01-01"), (year2 + "-12-31"), select_type1, "ASC", conn);
				} else if (select_type2.equals("TXN")) {
					income1 = Query.PharmachyCount((year1 + "-01-01"), (year1 + "-12-31"), select_type1, "All",
							conn);
					income2 = Query.PharmachyCount((year2 + "-01-01"), (year2 + "-12-31"), select_type1, "All",
							conn);
					al = Query.PharmachyCountBreakdownWC((year1 + "-01-01"), (year1 + "-12-31"),
							(year2 + "-01-01"), (year2 + "-12-31"), select_type1, "ASC", conn);
				}
		...
		Highcharts
				.chart(
						'containerBar',
						{
							...
							title : {
								text : '<%=select_type1%> <%=select_type2_S%> Comparison - Pharmacy'
							},
							subtitle : {
								text : '<%=title_ex%> <b><u><%=sub_title1%></u></b> and <b><u><%=sub_title2%></u></b> (With cashier discount)'
							},
							xAxis : {
								//gridLineColor: 'white',
								categories : [ '' ],
		...
		var chart2 = new Highcharts
				.chart(
						'containerBar1',
						{
							...
							colors: [null, null],
							title : {
								text : '<%=select_type1%> <%=select_type2_S%> Comparison - Pharmacy Breakdown',
								align: 'left'
							},
							subtitle : {
								text : '<%=title_ex%> <b><u><%=sub_title1%></u></b> and <b><u><%=sub_title2%></u></b> (Without cashier discount)',
								align: 'left'
							},
							xAxis : {
								min: 0,
							...
							plotOptions : {
								column : {
									...
									dataLabels : {
										...
										x : 0,
										y : -35,
										format : '<span style="font-size:18px; color:#f96868;">{point.per:.0f} {point.symbol}</span><br/><span style="font-size:10px"><%=curency1%>{point.y:,.<%=fraction%>}</span>'
									}
								}
							},
							"series" : [
			...
			<div class="row row-offcanvas row-offcanvas-right">
				...
				<div class="content-wrapper">
					...
					<jsp:include page="../../filter_all.jsp"><jsp:param
							value="comparison1.jsp" name="action" /><jsp:param
						...
						<div class="col-12 grid-margin"
							...
							<div class="card" style="">
								<div class="card-body" style="">
									<div id="containerBar1" style="width: 100%; height: 550px;"></div>
