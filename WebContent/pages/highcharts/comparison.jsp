...
<%
	int mc = 0;
		...
		double ph_target = 0, lab_target = 0, mp_target = 0, sna_target = 0, channel_target = 0;

		double ph_income1 = 0, ph_income2 = 0, lab_income1 = 0, lab_income2 = 0, mp_income1 = 0, mp_income2 = 0,
				sna_income1 = 0, sna_income2 = 0, channel_income1 = 0, channel_income2 = 0;
		...
		try {
			...
			if (date_selection.equals("date")) {
				...
				if (select_type1.equals("CHANNEL") || select_type1.equals("All")) {
					...
					da_al1 = RESTfulJerseyClient.getDoctorAppointmentList(mc, "TXN", access_token,  date1, date1);
					}
				}

				if (select_type1.equals("CHANNEL") || select_type1.equals("All")) {
					if(!access_token.equals("")){
					da_al2 = RESTfulJerseyClient.getDoctorAppointmentList(mc, "TXN", access_token,  date2, date2);
					}
				}
				...
				if (select_type2.equals("REV")) {
					...
					for (DoctorAppointment da : da_al1) {
						channel_income1 += da.getTotalAmount();
					}
					...
					for (DoctorAppointment da : da_al2) {
						channel_income2 += da.getTotalAmount();
					}
					...
					for (DoctorAppointment da : da_al1) {
						channel_income1 += da.getChannelCount();
					}
					...
					for (DoctorAppointment da : da_al2) {
						channel_income2 += da.getChannelCount();
					}
				}
				sub_title1 = date1;
				sub_title2 = date2;
				...
				lab_target = (Query.getTargetTransaction("LAB", week1, week11, select_type1, select_type2, conn)
						...
						/ 2;
				//mp_target = (Query.getTargetTransaction("MP", week1, week11, select_type1, select_type2, conn) + Query.getTargetTransaction("MP", week2, week22, select_type1, select_type2, conn)) / 2;
				...
				if (select_type1.equals("CHANNEL") || select_type1.equals("All")) {
					if(!access_token.equals("")){
					da_al1 = RESTfulJerseyClient.getDoctorAppointmentList(mc, "TXN", access_token,  week1, week11);
					}
				}

				if (select_type1.equals("CHANNEL") || select_type1.equals("All")) {
					if(!access_token.equals("")){
					da_al2 = RESTfulJerseyClient.getDoctorAppointmentList(mc, "TXN", access_token,  week2, week22);
					}
				}
				...
				//mp_target = Query.getTargetTransaction("MP", (month1 + "-01"), Query.getLastDate(month2, conn), select_type1, select_type2, conn) / 2;
				sna_target = Query.getTargetTransaction("SNA", (month1 + "-01"),
						Query.getLastDate(month2, conn), select_type1, select_type2, conn) / 2; */

				if (select_type1.equals("CHANNEL") || select_type1.equals("All")) {
					if(!access_token.equals("")){
					da_al1 = RESTfulJerseyClient.getDoctorAppointmentList(mc, "TXN", access_token,  (month1 + "-01"),
							Query.getLastDate(month1, conn));
					}
				}

				if (select_type1.equals("CHANNEL") || select_type1.equals("All")) {
					if(!access_token.equals("")){
					da_al2 = RESTfulJerseyClient.getDoctorAppointmentList(mc, "TXN", access_token,  (month2 + "-01"),
							Query.getLastDate(month2, conn));
					}
				}
				...
				if (select_type2.equals("REV")) {
					...
					lab_income1 = Query.LabCountWC((month1 + "-01"), Query.getLastDate(month1, conn),
							select_type1, "All", conn);
					//mp_income1 = Query.MediPackCountWC((month1 + "-01"), Query.getLastDate(month1, conn), select_type1, "All", conn);
					...
					lab_income2 = Query.LabCountWC((month2 + "-01"), Query.getLastDate(month2, conn),
							select_type1, "All", conn);
					//mp_income2 = Query.MediPackCountWC((month2 + "-01"), Query.getLastDate(month2, conn), select_type1, "All", conn);
				...
				lab_target = Query.getTargetTransaction("LAB", (year1 + "-01-01"), (year2 + "-12-31"),
						select_type1, select_type2, conn) / 2;
				//mp_target = Query.getTargetTransaction("MP", (year1 + "-01-01"), (year2 + "-12-31"), select_type1, select_type2, conn) / 2;
				sna_target = Query.getTargetTransaction("SNA", (year1 + "-01-01"), (year2 + "-12-31"),
						select_type1, select_type2, conn) / 2; */

				if (select_type1.equals("CHANNEL") || select_type1.equals("All")) {
					if(!access_token.equals("")){
					da_al1 = RESTfulJerseyClient.getDoctorAppointmentList(mc, "TXN", access_token,  (year1 + "-01-01"),
							(year1 + "-12-31"));
					}
				}
				...
				if (select_type1.equals("CHANNEL") || select_type1.equals("All")) {
					if(!access_token.equals("")){
					da_al2 = RESTfulJerseyClient.getDoctorAppointmentList(mc, "TXN", access_token,  (year2 + "-01-01"),
							(year2 + "-12-31"));
					}
				}
				...
				if (select_type2.equals("REV")) {
					...
					lab_income1 = Query.LabCountWC((year1 + "-01-01"), (year1 + "-12-31"), select_type1, "All",
							conn);
					//mp_income1 = Query.MediPackCountWC((year1 + "-01-01"), (year1 + "-12-31"), select_type1, "All", conn);
					sna_income1 = Query.ServiceNonAppCountWC((year1 + "-01-01"), (year1 + "-12-31"),
							select_type1, "All", conn);
					...
					lab_income2 = Query.LabCountWC((year2 + "-01-01"), (year2 + "-12-31"), select_type1, "All",
							conn);
					//mp_income2 = Query.MediPackCountWC((year2 + "-01-01"), (year2 + "-12-31"), select_type1, "All", conn);
					sna_income2 = Query.ServiceNonAppCountWC((year2 + "-01-01"), (year2 + "-12-31"),
							select_type1, "All", conn);
		...
		double tot_income = ph_income2 + lab_income2 + mp_income2 + sna_income2 + channel_income2;
		double tot_target = ph_income1 + lab_income1 + mp_income1 + sna_income1 + channel_income1;

		if (select_type1.equals("All")) {
			session.setAttribute("target_all", ph_target + lab_target + sna_target + mp_target + channel_target);
			session.setAttribute("revenue1", ph_income1 + lab_income1 + sna_income1 + mp_income1 + channel_income1);
			session.setAttribute("revenue2", ph_income2 + lab_income2 + sna_income2 + mp_income2 + channel_income2);
		}
		...
		if (channel_income1 < channel_income2) {
			color5_1 = "#F16868";
			color5_2 = "#64E572";
		} else {
			color5_1 = "#64E572";
		...
		var myChart = Highcharts
				.chart(
						'containerBar',
						{
							...
							colors: [null, null],
							title : {
								text : '<%=select_type1%> <%=select_type2_S%> Comparison - <%=title_ex%> - Without Collecting Center'
							},
							subtitle : {
							...
							xAxis : {
								...
								categories : [ 'Lab', 'Pharmacy',
										'Service Non<br/>Appointment', 'Channel' ]
							},
							yAxis : {
								//gridLineColor : 'white',
								title : {
...
"symbol" : null,
	...
	"color" : "<%=color5_2%>",
	"y" : <%=channel_income2%>,
	<%if (channel_income1 != 0) {%>
	"symbol" : "%",
	"per" : <%=channel_income2 * 100 / channel_income1%>
	<%} else {%>
	"symbol" : null,
	"per" : null
	<%}%>
