...
<%
	int mc = 0;
		...
		String select_type2_S = "Revenue", curency1 = "Rs. ", curency2 = "(Rs.)", curency3 = "",
				discount_s1 = "(With cashier discount)", discount_s2 = "(Without cashier discount)";
		...
		try {
			...
			if (date_selection.equals("date")) {
				...
				//target = Query.getTargetTransaction("LAB", date1, date1, select_type1, select_type2, conn);

				if (select_type2.equals("REV")) {
					income1 = Query.LabRevenueWC(date1, date1, select_type1, conn);
					income2 = Query.LabRevenueWC(date2, date2, select_type1, conn);
					al = Query.LabRevenueBreakdownWC(date1, date1, date2, date2, select_type1, "ASC", conn);
				} else if (select_type2.equals("TXN")) {
					income1 = Query.LabCountWC(date1, date1, select_type1, "All", conn);
					income2 = Query.LabCountWC(date2, date2, select_type1, "All", conn);
					al = Query.LabCountBreakdownWC(date1, date1, date2, date2, select_type1, "ASC", conn);
				}
				...
				/* target = (Query.getTargetTransaction("LAB", week1, week11, select_type1, select_type2, conn)
						+ Query.getTargetTransaction("LAB", week2, week22, select_type1, select_type2, conn))
						/ 2; */

				if (select_type2.equals("REV")) {
					income1 = Query.LabRevenueWC(week1, week11, select_type1, conn);
					income2 = Query.LabRevenueWC(week2, week22, select_type1, conn);
					al = Query.LabRevenueBreakdownWC(week1, week11, week2, week22, select_type1, "ASC", conn);
				} else if (select_type2.equals("TXN")) {
					income1 = Query.LabCountWC(week1, week11, select_type1, "All", conn);
					income2 = Query.LabCountWC(week2, week22, select_type1, "All", conn);
					al = Query.LabCountBreakdownWC(week1, week11, week2, week22, select_type1, "ASC", conn);
				}
				...
				/* target = Query.getTargetTransaction("LAB", (month1 + "-01"), Query.getLastDate(month2, conn),
						select_type1, select_type2, conn) / 2; */

				if (select_type2.equals("REV")) {
					income1 = Query.LabRevenueWC((month1 + "-01"), Query.getLastDate(month1, conn),
							select_type1, conn);
					income2 = Query.LabRevenueWC((month2 + "-01"), Query.getLastDate(month2, conn),
							select_type1, conn);
					al = Query.LabRevenueBreakdownWC((month1 + "-01"), Query.getLastDate(month1, conn),
							(month2 + "-01"), Query.getLastDate(month2, conn), select_type1, "ASC", conn);
				} else if (select_type2.equals("TXN")) {
					income1 = Query.LabCountWC((month1 + "-01"), Query.getLastDate(month1, conn), select_type1,
							"All", conn);
					income2 = Query.LabCountWC((month2 + "-01"), Query.getLastDate(month2, conn), select_type1,
							"All", conn);
					al = Query.LabCountBreakdownWC((month1 + "-01"), Query.getLastDate(month1, conn),
							(month2 + "-01"), Query.getLastDate(month2, conn), select_type1, "ASC", conn);
				}
				...
				/* target = Query.getTargetTransaction("LAB", (year1 + "-01-01"), (year2 + "-12-31"), select_type1,
						select_type2, conn) / 2; */

				if (select_type2.equals("REV")) {
					income1 = Query.LabRevenueWC((year1 + "-01-01"), (year1 + "-12-31"), select_type1, conn);
					income2 = Query.LabRevenueWC((year2 + "-01-01"), (year2 + "-12-31"), select_type1, conn);
					al = Query.LabRevenueBreakdownWC((year1 + "-01-01"), (year1 + "-12-31"), (year2 + "-01-01"),
							(year2 + "-12-31"), select_type1, "ASC", conn);
				} else if (select_type2.equals("TXN")) {
					income1 = Query.LabCountWC((year1 + "-01-01"), (year1 + "-12-31"), select_type1, "All",
							conn);
					income2 = Query.LabCountWC((year2 + "-01-01"), (year2 + "-12-31"), select_type1, "All",
							conn);
					al = Query.LabCountBreakdownWC((year1 + "-01-01"), (year1 + "-12-31"), (year2 + "-01-01"),
							(year2 + "-12-31"), select_type1, "ASC", conn);
				}
		...
		////////////////////////////////////////////////////////////////////////////////////
		// System.out.println(color1_1 + " , " + color1_2);
%>
<script>
	$(function() {
		...
		Highcharts
				.chart(
						'containerBar',
						{
							...
							title : {
								text : '<%=select_type1%> <%=select_type2_S%> Comparison - Lab'
							},
							subtitle : {
								text : '<%=title_ex%> <b><u><%=sub_title1%></u></b> and <b><u><%=sub_title2%></u></b> <%=discount_s1%>'
							},
							xAxis : {
								//gridLineColor: 'white',
								categories : [ '' ],
		...
		var chart2 = new Highcharts
				.chart(
						'containerBar1',
						{
							...
							colors: [null, null],
							title : {
								text : '<%=select_type1%> <%=select_type2_S%> Comparison - Lab Breakdown',
								align: 'left'
							},
							subtitle : {
								text : '<%=title_ex%> <b><u><%=sub_title1%></u></b> and <b><u><%=sub_title2%></u></b> <%=discount_s2%>',
								align: 'left'
							},
							xAxis : {
								min: 0,
								...
								<%if (al.size() > 0) {
					for (int i = 0; i < al.size(); i++) {
						if (al.get(i)[2] != null && (Double.parseDouble(al.get(i)[3]) + Double.parseDouble(al.get(i)[4])) > 0) {%>
								'<%=al.get(i)[2]%>',
								<%//if (i == 10) break;
						}
					}
			...
			<div class="row row-offcanvas row-offcanvas-right">
				...
				<div class="content-wrapper">
					...
					<jsp:include page="../../filter_all.jsp"><jsp:param
							value="comparison2.jsp" name="action" /><jsp:param
