		...
		var chart1 = Highcharts
				.chart(
						'containerBar1',
						{
							...
							title : {
								text : '<%=select_type1%> Target Vs Achievement'
							},
							subtitle : {
							...
							xAxis : {
								//gridLineColor : 'white',
								categories : [ '<%=arraylist1.get(0)[1]%>', '<%=arraylist1.get(1)[1]%>', '<%=arraylist1.get(2)[1]%>',
										 '<%=arraylist1.get(3)[1]%>'],
								title : {
									text : null
								}
							...
							plotOptions : {
								column : {
									...
									dataLabels : {
										...
										x: 0,
						                y: -60,
										format : '<font style="font-size:22px; color:#f96868;">{point.per:.0f} {point.symbol}</font><br/><span style="font-size:12px"><%=currency1%> {point.y}</span>'
									}
								}
							},
							"series" : [ {
							...
							plotOptions : {
								...
								series : {
									dataLabels : {
										allowOverlap : true,
										enabled : true,
										format : '<span style="font-size:12px;">{point.name}</span><br/><span style="font-size:21px; color:#f96868;">{point.y:.0f} %</span><br/><span style="font-size:11px"><%=currency1%> {point.amount:.0f}</span>'
									}
								}
							},
...
<%=sna_income%>
	,
	<%if(tot_income > 0){%>"y" :
<%=df.format((sna_income * 100) / tot_income)%>
	,<%}%>
...
<%=ph_income%>
	,
	<%if(tot_income > 0){%>"y" :
<%=df.format((ph_income * 100) / tot_income)%>
	,<%}%>
											"drilldown" : "Pharmacy"
										},
										{
...
<%=mp_income%>
	,
	<%if(tot_income > 0){%>"y" :
<%=df.format((mp_income * 100) / tot_income)%>
	,<%}%>
											"drilldown" : "Medical Package"
										},
										{
											"code" : "LAB",
											"name" : "Lab",
...
<%=lab_income%>
	,
	<%if(tot_income > 0){%>"y" :
<%=df.format((lab_income * 100) / tot_income)%>
	,<%}%>
											"drilldown" : "Lab"
										} ]
							} ],
								...
								"series" : [
										{
											...
											"data" : [
												...
												if(select_type2.equals("REV")){
													alSNA = Query.ServiceNonAppBreakdown(date_from, date_to, select_type1, "DESC", "", conn);
												}else if(select_type2.equals("TXN")){
													alSNA = Query.ServiceNonAppCountBreakdown(date_from, date_to, select_type1, "DESC", "", conn);
												}
										if(alSNA.size()>0){
				...
				if(i==9) break;									
										...
										},
										{
											...
											"data" : [
												...
												if(select_type2.equals("REV")){
													alP = Query.PharmacyBreakdown(date_from, date_to, select_type1, "DESC", "", conn);
												}else if(select_type2.equals("TXN")){
													alP = Query.PharmacyCountBreakdown(date_from, date_to, select_type1, "DESC", "", conn);
												}
			...
			,<%}%>
										...
										},
										{
											...
											"data" : [
												...
												if(select_type2.equals("REV")){
													alMP = Query.MediPackBreakdown(date_from, date_to, select_type1, "DESC", "", conn);
												}else if(select_type2.equals("TXN")){
													alMP = Query.MediPackCountBreakdown(date_from, date_to, select_type1, "DESC", "", conn);
												}
					...
					%>
															{
																...
																<%}%>
																<%if(mp_income > 0){%>"y" :
		<%=df.format(Double.parseDouble(alMP.get(i)[1]) * 100 / mp_income)%>
			,<%}%>
										...
										},
										{
											...
											"data" : [
												...
												if(select_type2.equals("REV")){
													alL = Query.LabBreakdown(date_from, date_to, select_type1, "DESC", "", conn);
												}else if(select_type2.equals("TXN")){
													alL = Query.LabCountBreakdown(date_from, date_to, select_type1, "DESC", "", conn);
												}
...
<%=mp_target%>
	<%if(tot_target > 0){%>,
											y :
<%=df.format((mp_target * 100) / tot_target)%><%}%>
	},
										{
											code : 'LAB',
											name : 'Lab',
