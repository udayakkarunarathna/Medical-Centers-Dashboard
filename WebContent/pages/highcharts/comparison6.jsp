			...
			if (date_selection.equals("date")) {
				...
				if (select_type2.equals("REV")) {
					for (DoctorAppointment da : da_al1) {
						income1 += da.getTotalAmount();
					}
					for (DoctorAppointment da : da_al2) {
						income2 += da.getTotalAmount();
					}
					for (DoctorAppointment da : da_al1) {
						String[] a = new String[3];
						a[0] = date1;
						a[1] = da.getDoctorName();
						a[2] = Double.toString(da.getTotalAmount());
						al1.add(a);
					}
					for (String[] doc : al1) {
						String[] obj = {date1, date2, doc[1], doc[2], "0"};
						if (hashMap.containsKey(doc[1])) {
							hashMap.get(doc[1])[3] = Double.toString(
									Double.parseDouble(hashMap.get(doc[1])[3]) + Double.parseDouble(doc[2]));
						} else {
							hashMap.put(doc[1], obj);
						}
					}
					for (DoctorAppointment da : da_al2) {
						String[] a = new String[3];
					...
					for (String[] doc : al2) {
						if (hashMap.containsKey(doc[1])) {
							hashMap.get(doc[1])[4] = Double.toString(
									Double.parseDouble(hashMap.get(doc[1])[4]) + Double.parseDouble(doc[2]));
						} else {
							String[] obj = {date1, date2, doc[1], "0", doc[2]};
							hashMap.put(doc[1], obj);
						}
					}
				} else if (select_type2.equals("TXN")) {
					for (DoctorAppointment da : da_al1) {
						income1 += da.getChannelCount();
					}
					for (DoctorAppointment da : da_al2) {
						income2 += da.getChannelCount();
					}
					for (DoctorAppointment da : da_al1) {
						String[] a = new String[3];
						a[0] = date1;
						a[1] = da.getDoctorName();
						a[2] = Double.toString(da.getChannelCount());
						al1.add(a);
					}
					for (String[] doc : al1) {
						String[] obj = {date1, date2, doc[1], doc[2], "0"};
				...
				if (select_type2.equals("REV")) {
					...
					for (String[] doc : al1) {
						String[] obj = {week1 + " - " + week11, week2 + " - " + week22, doc[1], doc[2], "0"};
						if (hashMap.containsKey(doc[1])) {
							hashMap.get(doc[1])[3] = Double.toString(
									Double.parseDouble(hashMap.get(doc[1])[3]) + Double.parseDouble(doc[2]));
						} else {
					...
					for (String[] doc : al2) {
						if (hashMap.containsKey(doc[1])) {
							...
							String[] obj = {week1 + " - " + week11, week2 + " - " + week22, doc[1], "0",
									doc[2]};
							hashMap.put(doc[1], obj);
						}
					}
				...
				if (select_type2.equals("REV")) {
					...
					for (DoctorAppointment da : da_al1) {
						String[] a = new String[3];
						a[0] = month1 + "-01" + " - " + Query.getLastDate(month1, conn);
						a[1] = da.getDoctorName();
						a[2] = Double.toString(da.getTotalAmount());
						al1.add(a);
					}
					for (String[] doc : al1) {
						String[] obj = {month1 + "-01" + " - " + Query.getLastDate(month1, conn),
								month2 + "-01" + " - " + Query.getLastDate(month2, conn), doc[1], doc[2], "0"};
						if (hashMap.containsKey(doc[1])) {
							hashMap.get(doc[1])[3] = Double.toString(
									Double.parseDouble(hashMap.get(doc[1])[3]) + Double.parseDouble(doc[2]));
						} else {
					...
					for (DoctorAppointment da : da_al2) {
						String[] a = new String[3];
						a[0] = month2 + "-01" + " - " + Query.getLastDate(month2, conn);
						a[1] = da.getDoctorName();
						a[2] = Double.toString(da.getTotalAmount());
						al2.add(a);
					}
					for (String[] doc : al2) {
						if (hashMap.containsKey(doc[1])) {
							...
							String[] obj = {month1 + "-01" + " - " + Query.getLastDate(month1, conn),
									month2 + "-01" + " - " + Query.getLastDate(month2, conn), doc[1], "0",
									doc[2]};
							hashMap.put(doc[1], obj);
						}
					}
				...
				if (select_type2.equals("REV")) {
					...
					for (DoctorAppointment da : da_al1) {
						String[] a = new String[3];
						a[0] = year1 + "-01-01" + " - " + year1 + "-12-31";
						a[1] = da.getDoctorName();
						a[2] = Double.toString(da.getTotalAmount());
						al1.add(a);
					}
					for (String[] doc : al1) {
						String[] obj = {year1 + "-01-01" + " - " + year1 + "-12-31",
								year2 + "-01-01" + " - " + year2 + "-12-31", doc[1], doc[2], "0"};
						if (hashMap.containsKey(doc[1])) {
							hashMap.get(doc[1])[3] = Double.toString(
									Double.parseDouble(hashMap.get(doc[1])[3]) + Double.parseDouble(doc[2]));
						} else {
					...
					for (String[] doc : al2) {
						if (hashMap.containsKey(doc[1])) {
							...
							String[] obj = {year1 + "-01-01" + " - " + year1 + "-12-31",
									year2 + "-01-01" + " - " + year2 + "-12-31", doc[1], "0", doc[2]};
							hashMap.put(doc[1], obj);
						}
					}
				} else if (select_type2.equals("TXN")) {
		...
		Highcharts
				.chart(
						'containerBar',
						{
							...
							title : {
								text : '<%=select_type1%> <%=select_type2_S%> Comparison - Channel'
							},
							subtitle : {
		...
		var chart2 = new Highcharts
				.chart(
						'containerBar1',
						{
							...
							colors: [null, null],
							title : {
								text : '<%=select_type1%> <%=select_type2_S%> Comparison - Channel Breakdown',
								align: 'left'
							},
							subtitle : {
								text : '<%=title_ex%> <b><u><%=sub_title1%></u></b> and <b><u><%=sub_title2%></u></b>',
							...
							plotOptions : {
								column : {
									...
									dataLabels : {
										...
										x : 0,
										y : -35,
										format : '<span style="font-size:18px; color:#f96868;">{point.per:.0f} {point.symbol}</span><br/><span style="font-size:12px"><%=curency1%>{point.y:,.0f}</span>'
									}
								}
							},
							"series" : [
									{
										...
										"data" : [ 
											<%String color = "";
				for (String[] array : hashMap.values()) {
					color = "";
					if (Double.parseDouble(array[3]) < Double.parseDouble(array[4])) {
						color = "#F16868"; // green
					} else {
						color = "#64E572"; // red
					}%>
					...
					if (Double.parseDouble(array[3]) < Double.parseDouble(array[4])) {
						...
						color = "#F16868"; // green
													...
													{																								
														"color" : "<%=color%>",
														"y" : <%=array[4]%>,
														<%if (Double.parseDouble(array[3]) > 0) {%>
														"per" : <%=Double.parseDouble(array[4]) * 100 / Double.parseDouble(array[3])%>,
														"symbol" : '%'
														<%} else {%>
														"per" : null,
														"symbol" : null
			...
			<div class="row row-offcanvas row-offcanvas-right">
				...
				<div class="content-wrapper">
					...
					<jsp:include page="../../filter_all.jsp"><jsp:param
							value="comparison6.jsp" name="action" /><jsp:param
					...
					<div class="row">
						<div class="col-12 grid-margin"
							style="overflow: scroll; min-width: 100%; width: <%=hashMap.size() * 100%>px;">
							<div class="card"
								style="min-width: 100%; width: <%=hashMap.size() * 100%>px;">
								<div class="card-body"
									style="min-width: 100%; width: <%=hashMap.size() * 100%>px;">
									<div id="containerBar1" style="min-width: 100%; height: 500px; width: <%=hashMap.size() * 100%>px;"></div>
